...
class Kinova_MJ(object): #core of mujoco simulation
    def __init__(self):
    ...
    def init_camera(self):
        self._viewer.cam.lookat[:] = [-0.05814825, -0.45016306, -0.01190703]
        self._viewer.cam.azimuth = -547.7087124
...
def read_ang_data_v(filename): #angle data reading
    f = open(filename, "r")
    if f.mode == 'r':
        angle_data = f.read()
...
def grab_cuting(target, K, sim_img, real, nwmask=[], save_fl=0):
    ...
    mask_erod = cv2.dilate(mj_seg, kernel, iterations=8)  # using dia as erod since the color
...
def seg_process(d_pos, d_rot, filename, sim, target, imag_save_fl=0, rotation_fl=0, indata=[]):
    ...
    for i in range(len(bb)):
        ...
        realim, mj = grab_cuting(target, i, ori_mask, realimg, save_fl=1)
